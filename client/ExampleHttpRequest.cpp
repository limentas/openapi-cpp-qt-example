...
ExampleHttpRequestInput::ExampleHttpRequestInput() {
    initialize();
}

ExampleHttpRequestInput::ExampleHttpRequestInput(QString v_url_str, QString v_http_method) {
    initialize();
    url_str = v_url_str;
    http_method = v_http_method;
}

void ExampleHttpRequestInput::initialize() {
    var_layout = NOT_SET;
    url_str = "";
    http_method = "GET";
}

void ExampleHttpRequestInput::add_var(QString key, QString value) {
    vars[key] = value;
}

void ExampleHttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, QString mime_type) {
    ExampleHttpFileElement file;
...
ExampleHttpRequestWorker::ExampleHttpRequestWorker(QObject *parent, QNetworkAccessManager *_manager)
...
ExampleHttpRequestWorker::~ExampleHttpRequestWorker() {
...
QMap<QString, QString> ExampleHttpRequestWorker::getResponseHeaders() const {
    return headers;
}

ExampleHttpFileElement ExampleHttpRequestWorker::getHttpFileElement(const QString &fieldname) {
    if (!files.isEmpty()) {
    ...
    return ExampleHttpFileElement();
}

QByteArray *ExampleHttpRequestWorker::getMultiPartField(const QString &fieldname) {
...
void ExampleHttpRequestWorker::setTimeOut(int timeOutMs) {
...
void ExampleHttpRequestWorker::setWorkingDirectory(const QString &path) {
    if (!path.isEmpty()) {
        workingDirectory = path;
    }
}

void ExampleHttpRequestWorker::setResponseCompressionEnabled(bool enable) {
    isResponseCompressionEnabled = enable;
}

void ExampleHttpRequestWorker::setRequestCompressionEnabled(bool enable) {
    isRequestCompressionEnabled = enable;
}

int  ExampleHttpRequestWorker::getHttpResponseCode() const{
    return httpResponseCode;
}

QString ExampleHttpRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
...
void ExampleHttpRequestWorker::execute(ExampleHttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        // add files
        for (QList<ExampleHttpFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
    ...
    if (input->request_body.size() > 0) {
        ...
        if(!isFormData && (input->var_layout != MULTIPART) && isRequestCompressionEnabled){
            request_content.append(compress(input->request_body, 7, ExampleCompressionType::Gzip));
        } else {
    ...
    if (ExampleHttpRequestWorker::sslDefaultConfiguration != nullptr) {
        request.setSslConfiguration(*ExampleHttpRequestWorker::sslDefaultConfiguration);
    }
...
void ExampleHttpRequestWorker::on_reply_finished(QNetworkReply *reply) {
    bool codeSts = false;
...
void ExampleHttpRequestWorker::on_reply_timeout(QNetworkReply *reply) {
...
void ExampleHttpRequestWorker::process_response(QNetworkReply *reply) {
    ...
    if (!contentDispositionHdr.isEmpty()) {
        ...
        if ((contentDisposition.count() > 0) && (contentDisposition.first() == QString("attachment"))) {
            ...
            ExampleHttpFileElement felement;
...
QByteArray ExampleHttpRequestWorker::decompress(const QByteArray& data){
    
    Q_UNUSED(data);
    return QByteArray();
}

QByteArray ExampleHttpRequestWorker::compress(const QByteArray& input, int level, ExampleCompressionType compressType) {
...
QSslConfiguration *ExampleHttpRequestWorker::sslDefaultConfiguration;
